...
class ServiceCommunicationTestController extends Controller
{
    ...
    public function testContentTokenValidation(Request $request): JsonResponse
    {
        try {
            $token = $request->bearerToken();
            ...
            return response()->json([
                'success' => true,
                'message' => 'Token validation successful in content service',
                'content_service_status' => 'connected',
                'user_data' => $user,
                'token_info' => [
            ...
            return response()->json([
                'success' => false,
                'message' => 'Content token validation test failed',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    public function testServiceIntegration(Request $request): JsonResponse
    {
        try {
            ...
            // Test if other services can access content service endpoints
            $contentServiceUrl = env('APP_URL', 'http://fitnease-content');
            try {
                $contentResponse = Http::timeout(10)->withHeaders([
                    'Authorization' => 'Bearer ' . $token,
                    'Accept' => 'application/json'
                ])->get($contentServiceUrl . '/api/content/exercises');

                $testResults['content_service_access'] = [
                    'exercises_accessible' => $contentResponse->successful() ? 'success' : 'failed',
                    'response_code' => $contentResponse->status(),
                    'service_response' => $contentResponse->successful() ? 'accessible' : 'rejected'
                ];
            } catch (\Exception $e) {
                $testResults['content_service_access'] = [
                    'exercises_accessible' => 'error',
                    'error' => $e->getMessage()
                ];
            }
            ...
            $mediaServiceUrl = env('MEDIA_SERVICE_URL', 'http://fitnease-media');
            try {
                $mediaResponse = Http::timeout(10)->withHeaders([
                    'Authorization' => 'Bearer ' . $token,
                    'Accept' => 'application/json'
                ])->get($mediaServiceUrl . '/api/health');

                $testResults['media_service'] = [
                    'connectivity' => $mediaResponse->successful() ? 'success' : 'failed',
